...
const PermissionList: React.FC<any> = ({
  filter,
  permissions,
  isLoadingPermission,
}) => {
  const [formPermissionList] = Form.useForm();
  ...
  const updatePermissionList = (values: any, title: any) => {
    ...
    formPermissionList.setFieldsValue({ items: newPermissionList });
  ...
  useEffect(() => {
    formPermissionList.setFieldsValue({ items: permissions });
