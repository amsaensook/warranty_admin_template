...
import { Button, Radio, Form, Select, Divider, Checkbox } from "antd";
...
const Permission: React.FC<any> = () => {
  ...
  const onFormLayoutChange = (value: any) => {
    if (value?.User_Group) {
      formPermission.setFieldsValue({ User_Group_Value: null });
  ...
  return (
    <>
      ...
      <PermissionList
        filter={
          formPermission.getFieldValue("User_Group_Value") && permission
        }
        permissions={
          formPermission.getFieldValue("User_Group_Value") &&
